    ...
    public void onSetUpView() {
        ...
        if(emojicon != null){
            if(emojicon.getBigIcon() != 0){
                Glide.with(activity).load(emojicon.getBigIcon()).apply(requestOptions).apply(requestOptions).into(imageView);
//                Glide.with(activity).load(emojicon.getBigIcon()).placeholder(R.drawable.ease_default_expression).into(imageView);
            }else if(emojicon.getBigIconPath() != null){
                Glide.with(activity).load(emojicon.getBigIconPath()).apply(requestOptions).apply(requestOptions).into(imageView);
