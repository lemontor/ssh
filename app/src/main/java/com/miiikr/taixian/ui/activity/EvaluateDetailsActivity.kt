...
class EvaluateDetailsActivity : BaseMvpActivity<DetailsPresenter>(), DetailsView {

    override fun <T : Any> onSuccess(responseId: Int, response: T) {
        if (RequestInterface.REQUEST_EVA_DETAILS_ID == responseId) {
    ...
    private fun initListener() {
        ...
        mTvSell.setOnClickListener {
            IntentUtils.toRecover(this,productId)
        }
//        mPresenter.getEvaDetailsInfo(RequestInterface.REQUEST_EVA_DETAILS_ID, SharedPreferenceUtils(this).getValue(SharedPreferenceUtils.PREFERENCE_U_I)!!, productId, categoryId)
        mPresenter.getEvaDetailsInfo(RequestInterface.REQUEST_EVA_DETAILS_ID, "10086", productId, categoryId)
    ...
    private fun initUI() {
        ...
        mSSHProgressHUD = SSHProgressHUD.getInstance(this@EvaluateDetailsActivity)
